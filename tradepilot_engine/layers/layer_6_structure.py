...
UPDATED: Added pivot arrays and calculated facts for AI analysis
"""
import pandas as pd
import numpy as np
...
class Layer6Structure:
    ...
    - NEW: Pivot arrays and calculated facts for AI
    """
    ...
    def __init__(self):
        ...
        # NEW: Pivot trend settings
        self.pivot_trend_count = 5  # How many recent pivots to analyze
    ...
    def analyze(self, df: pd.DataFrame) -> Dict:
        ...
        Returns:
            Dict with RAW CHoCH, BOS, OB, FVG, liquidity data, and pivot trend facts
        """
        if len(df) < 50:
        ...
        # NEW: Calculate pivot trend facts
        current_price = df['close'].iloc[-1]
        pivot_trend_facts = self._calculate_pivot_trends(pivot_data, current_price)
        ...
        return {
            ...
            # ==================== NEW: PIVOT ARRAYS ====================
            "recent_pivot_highs": pivot_trend_facts["recent_highs"],
            "recent_pivot_lows": pivot_trend_facts["recent_lows"],
            "recent_pivot_high_indices": pivot_trend_facts["recent_high_indices"],
            "recent_pivot_low_indices": pivot_trend_facts["recent_low_indices"],

            # ==================== NEW: CALCULATED FACTS ====================
            # Consecutive counts (math done, AI interprets)
            "consecutive_higher_highs": pivot_trend_facts["consecutive_higher_highs"],
            "consecutive_higher_lows": pivot_trend_facts["consecutive_higher_lows"],
            "consecutive_lower_highs": pivot_trend_facts["consecutive_lower_highs"],
            "consecutive_lower_lows": pivot_trend_facts["consecutive_lower_lows"],

            # Pattern facts (true/false)
            "highs_ascending": pivot_trend_facts["highs_ascending"],
            "lows_ascending": pivot_trend_facts["lows_ascending"],
            "highs_descending": pivot_trend_facts["highs_descending"],
            "lows_descending": pivot_trend_facts["lows_descending"],

            # ==================== NEW: PRICE POSITION FACTS ====================
            "price_above_last_pivot_high": pivot_trend_facts["price_above_last_high"],
            "price_below_last_pivot_high": pivot_trend_facts["price_below_last_high"],
            "price_above_last_pivot_low": pivot_trend_facts["price_above_last_low"],
            "price_below_last_pivot_low": pivot_trend_facts["price_below_last_low"],
            "price_above_all_recent_lows": pivot_trend_facts["price_above_all_recent_lows"],
            "price_below_all_recent_highs": pivot_trend_facts["price_below_all_recent_highs"],

            # ==================== NEW: DISTANCE FACTS ====================
            "distance_to_last_pivot_high": pivot_trend_facts["distance_to_last_high"],
            "distance_to_last_pivot_high_pct": pivot_trend_facts["distance_to_last_high_pct"],
            "distance_to_last_pivot_low": pivot_trend_facts["distance_to_last_low"],
            "distance_to_last_pivot_low_pct": pivot_trend_facts["distance_to_last_low_pct"],

            # ==================== NEW: SWING RANGE ====================
            "current_swing_range": pivot_trend_facts["swing_range"],
            "current_swing_range_pct": pivot_trend_facts["swing_range_pct"],
            "price_position_in_range_pct": pivot_trend_facts["price_position_in_range_pct"],
            ...
            "price_vs_trend_ema_pct": round(float((df['close'].iloc[-1] - trend_ema.iloc[-1]) / trend_ema.iloc[-1] * 100), 2),
    ...
    # ==================== NEW: PIVOT TREND ANALYSIS ====================

    def _calculate_pivot_trends(self, pivot_data: Dict, current_price: float) -> Dict:
        """
        Calculate pivot trend facts - RAW FACTS only, no interpretation
        
        AI will look at these numbers and decide what they mean.
        """
        pivot_highs = pivot_data["pivot_highs"]
        pivot_lows = pivot_data["pivot_lows"]
        pivot_high_indices = pivot_data["pivot_high_indices"]
        pivot_low_indices = pivot_data["pivot_low_indices"]

        # Get recent pivots (last N)
        n = self.pivot_trend_count
        recent_highs = pivot_highs[-n:] if len(pivot_highs) >= n else pivot_highs
        recent_lows = pivot_lows[-n:] if len(pivot_lows) >= n else pivot_lows
        recent_high_indices = pivot_high_indices[-n:] if len(pivot_high_indices) >= n else pivot_high_indices
        recent_low_indices = pivot_low_indices[-n:] if len(pivot_low_indices) >= n else pivot_low_indices

        # Round for clean output
        recent_highs = [round(h, 2) for h in recent_highs]
        recent_lows = [round(l, 2) for l in recent_lows]

        # Calculate consecutive higher/lower counts
        consecutive_higher_highs = self._count_consecutive_ascending(recent_highs)
        consecutive_higher_lows = self._count_consecutive_ascending(recent_lows)
        consecutive_lower_highs = self._count_consecutive_descending(recent_highs)
        consecutive_lower_lows = self._count_consecutive_descending(recent_lows)

        # Are all recent pivots ascending/descending?
        highs_ascending = len(recent_highs) > 1 and all(
            recent_highs[i] > recent_highs[i-1] for i in range(1, len(recent_highs))
        )
        lows_ascending = len(recent_lows) > 1 and all(
            recent_lows[i] > recent_lows[i-1] for i in range(1, len(recent_lows))
        )
        highs_descending = len(recent_highs) > 1 and all(
            recent_highs[i] < recent_highs[i-1] for i in range(1, len(recent_highs))
        )
        lows_descending = len(recent_lows) > 1 and all(
            recent_lows[i] < recent_lows[i-1] for i in range(1, len(recent_lows))
        )
        ...
        price_above_last_high = current_price > last_high if last_high else None
        price_below_last_high = current_price < last_high if last_high else None
        price_above_last_low = current_price > last_low if last_low else None
        price_below_last_low = current_price < last_low if last_low else None

        price_above_all_recent_lows = all(current_price > l for l in recent_lows) if recent_lows else None
        price_below_all_recent_highs = all(current_price < h for h in recent_highs) if recent_highs else None
        ...
        price_position_in_range_pct = None
        ...
        return {
            ...
            "recent_high_indices": recent_high_indices,
            "recent_low_indices": recent_low_indices,

            # Consecutive counts
            "consecutive_higher_highs": consecutive_higher_highs,
            ...
            # Pattern facts
            "highs_ascending": highs_ascending,
            "lows_ascending": lows_ascending,
            "highs_descending": highs_descending,
            "lows_descending": lows_descending,

            # Price position
            "price_above_last_high": price_above_last_high,
            "price_below_last_high": price_below_last_high,
            "price_above_last_low": price_above_last_low,
            "price_below_last_low": price_below_last_low,
            "price_above_all_recent_lows": price_above_all_recent_lows,
            "price_below_all_recent_highs": price_below_all_recent_highs,

            # Distances
            "distance_to_last_high": distance_to_last_high,
            "distance_to_last_high_pct": distance_to_last_high_pct,
            "distance_to_last_low": distance_to_last_low,
            "distance_to_last_low_pct": distance_to_last_low_pct,

            # Range
            "swing_range": swing_range,
            "swing_range_pct": swing_range_pct,
            "price_position_in_range_pct": price_position_in_range_pct,
        }

    def _count_consecutive_ascending(self, values: List[float]) -> int:
        """Count consecutive ascending values from the end"""
        if len(values) < 2:
            return 0
    ...
    def _count_consecutive_descending(self, values: List[float]) -> int:
        """Count consecutive descending values from the end"""
        if len(values) < 2:
            return 0
    ...
    def _empty_result(self, reason: str) -> Dict:
        ...
        return {
            ...
            "recent_pivot_highs": [],
            "recent_pivot_lows": [],
            "recent_pivot_high_indices": [],
            "recent_pivot_low_indices": [],
            ...
            "price_above_last_pivot_high": None,
            "price_below_last_pivot_high": None,
            "price_above_last_pivot_low": None,
            "price_below_last_pivot_low": None,
            "price_above_all_recent_lows": None,
            "price_below_all_recent_highs": None,
            "distance_to_last_pivot_high": None,
            "distance_to_last_pivot_high_pct": None,
            "distance_to_last_pivot_low": None,
            "distance_to_last_pivot_low_pct": None,
            "current_swing_range": None,
            "current_swing_range_pct": None,
            "price_position_in_range_pct": None,
            # Original fields
            "choch_bull_detected": False,
            "choch_bear_detected": False,
